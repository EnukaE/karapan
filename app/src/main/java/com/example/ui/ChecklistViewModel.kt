...
class ChecklistViewModel(
    ...
    fun addChecklistShortcutToToday(checklistId: Int) {
        viewModelScope.launch {
            ...
            val list = lists.find { it.id == checklistId } ?: return@launch
            val shortcutText = "[CL_SHORTCUT:$checklistId] ${list.name}"
            val alreadyExists = repository.getItemsForChecklistDirect(todayId).any { it.text.startsWith("[CL_SHORTCUT:$checklistId]") }
            if (!alreadyExists) {
